...
class NoteInput extends HTMLElement {
  connectedCallback() {
    this.innerHTML = `
      <div>
        <textarea id="noteTitle" name="story" rows="2" cols="33" placeholder="Judul"></textarea>
        <textarea id="noteBody" name="story" rows="10" cols="50" placeholder="Deskripsi"></textarea>
      </div>
      <button class="add-note" onclick="addNote()"><i class="fas fa-plus"></i> Add Note</button>
    `;
  }
}
...
function addNote() {
  ...
  if (noteTitle && noteBody) {
    ...
    Swal.fire({
      icon: "error",
      title: "Oops...",
      text: "Mohon tambahkan judul dan isi.",
    });
  }
}
...
function editNote(id) {
  ...
  Swal.fire({
    title: "Edit Catatan",
    html: `
      <input id="editedTitle" class="swal2-input" value="${title}" placeholder="Judul">
      <textarea id="editedBody" class="swal2-textarea" placeholder="Isi Catatan">${body}</textarea>
    `,
    ...
    preConfirm: () => {
      ...
      if (!editedTitle || !editedBody) {
        Swal.showValidationMessage(
          "Silakan masukkan judul dan isi untuk catatan."
        );
      }
